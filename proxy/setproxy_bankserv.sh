...
set_parameters() {
	...
	if [[ $use_auth == 'y' ]]; then
		read -p "Enter Proxy Username   : " username1
		username=${username1/\\/\\\\}
		#echo -n $username
...
s      : Set proxy for Selby BankservAfrica
r      : Set proxy for Randburg BankservAfrica
b			 : Set proxy for backdoor in BankservAfrica
q      : Quit this program
...
read -p "modify /etc/environment ? (y/n)  " modifyenvironment
